...
如此循环操作，直至所有测试用例执行完成。
 ...
 ![](file:////tmp/wps-mikigo/ksohtml/wpsT0Y2yV.jpg)
...
_import _os\
test\_case\_list = \[ ]\
_for _root, dir, files _in _os.walk(cases\_path):\
    _for _file _in _files:\
        _if _file.startswith("test") _and _file.endswith(".py"):\
            test\_case\_list.append(file)
...
```shell
ssh user\@ip "cd cases/ && pytest test\_cases.py"
```
